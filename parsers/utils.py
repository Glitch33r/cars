...
class CheckUserFilters:
    ...
    def start(self):
        if self.is_update:
            print(f'hi car {self.car.model.name}')
...
def get_model_id(mark, model):
    if mark and model:
        ...
        if mark_id:
            model = re.sub(r"^(\-gruz|\-pass|\-1)$", "", model)
            ...
            if mark_id == 4:
                ...
                model = re.sub(r"(-ev|-1|-2|-gruz-pass|-gruz|-pass|-express)", "", model)
            elif mark_id == 36:
                model = re.sub(r"300(c|m|s)", "300-\1", model)
                model = re.sub(r"(-3|-2)", "", model)
            elif mark_id == 37:
                model = re.sub(r"citroen-", "", model)
                model = re.sub(r"(-mcv|-stepway|-gruz-pass|-gruz|-pass|-1|-van|-aircross|-vp|-vu)", "", model)
            elif mark_id == 38:
                model = re.sub(r"-1", "", model)
            elif mark_id == 40:
                model = re.sub(r"(-sens|-hatchback|-furgon|-pick-up|-sedan|-1|-2|-3|-sx)", "", model)
            elif mark_id == 51:
                ...
                model = re.sub(r"(-2|-abarth|-gruz-pass|-gruz|-pass|-1)", "", model)
            elif mark_id == 62:
                model = re.sub(r"(-gruz-pass|-gruz|-pass|-1|-2|-3)", "", model)
                ...
                model = re.sub(r"(-starex|-7|-1|-gruz-pass|-gruz|-pass|-electric|-coupe|-fastback)", "", model)
                model = re.sub(r"^h$", "h1", model)
                model = re.sub(r"^h([0-9]{3})$", "h-\1", model)
                model = re.sub(r"^.*?ix55.*?$", "ix55", model)
                model = re.sub(r"santafe", "santa-fe", model)
            elif mark_id == 86:
                model = re.sub(r"^(qx|q|m|g)-([0-9]{2})$", "\1\2", model)
                ...
                model = re.sub(r"(-ev|-gruz-pass|-pass|-gruz|-ii)", "", model)
            elif mark_id == 109:
                ...
                model = re.sub(r"(-seriya|-450-h|-450d|-1|-460|-470|-200t|-200h|-300h|-350|-250h)", "", model)
            elif mark_id == 122:
                model = re.sub(r"(-gruz-pass|-gruz|-pass)", "", model)
                ...
                model = re.sub(r"cx([0-9])", "cx-\1", model)
            elif mark_id == 125:
                model = re.sub(r"(-gruz-pass|-gruz|-pass|-coupe|-roadster-r190|-c-190)", "", model)
                ...
                model = re.sub(r"^(gl|glk|gls|ml|g|sl|slk|cls|clk|clc|cla|a|b|cl|s|r|v)-[0-9]{3}", "\1-class", model)
                model = re.sub(r"^(gl|glk|gls|ml|g|sl|slk|cls|clk|clc|cla|a|b|cl|s|r|v)$", "\1-class", model)
                model = re.sub(r"^(gl|glk|gls|ml|g|sl|slk|cls|clk|clc|cla|a|b|cl|s|r|v)\-[0-9]{2}\-amg", "\1-class",
                               model)
                model = re.sub(r"^(gls|s)-[0-9]{2}", "\1-class", model)
            elif mark_id == 128:
                model = re.sub(r"(-2|-cross)", "couper", model)
            elif mark_id == 130:
                model = re.sub(r"cooper(-hatch|-countryman|-clubman)", "couper", model)
                model = re.sub(r"cooper-s(-hatch|-countryman|-clubman)", "couper-s", model)
                model = re.sub(r"cooper-d(-hatch|-countryman|-clubman)", "couper-d", model)
            elif mark_id == 131:
                ...
                model = re.sub(r"^l([0-9]00)$", "l-\1", model)
                model = re.sub(r"(-gruz-pass|-gruz|-pass|-1)", "", model)
            elif mark_id == 135:
                model = re.sub(r"(-gruz-pass|-gruz|-pass|-gr|e-|-ii|-1|-bluebird|-sunny|-nismo-rs)", "", model)
                ...
                model = re.sub(r"(-3|-sports-tourer|-1|-2|-gruz-pass|-gruz|-pass|-x|-g|-4)", "", model)
            elif mark_id == 146:
                model = re.sub(
                    r"(-gruz-pass|-gruz|-pass|-tepee|-sedan|-gti|-1|-2|-sw|-cc|-ss|-hatchback-5d|-hatchback-3d)", "",
                    model)
                ...
                model = re.sub(r"(-hybrid|-plus|-e|-ev|-gruz-pass|-gruz|-pass|-76|-80|-90|-1)", "", model)
                ...
                model = re.sub(
                    r"(-sedan|-variant|-hatchback|-kombi-maxi|-kasten-maxi|-kasten|-maxi|-kombi|-allspace|t4-|t5-|t6-|t6-1-|-gruz-pass|-gruz|-pass|-alltrack|-1|e-)",
                    "", model)
                ...
                model = re.sub(r"(-zim|lada-)", "", model)
                model = re.sub(r"2790", "gazel", model)
                model = re.sub(r"^(21|23|24|24[0-9]{2}|3110[0-9]?|3109|(31029|24|3110|31105|21)-volga)$", "volga",
                               model)
            elif mark_id == 236:
                ...
                model = re.sub(r"(-cargo|-nova|1122-|-1)", "", model)
            elif mark_id == 240:
                pass
